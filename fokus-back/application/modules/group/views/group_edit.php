<div class="m-portlet m-portlet--creative m-portlet--first m-portlet--bordered-semi">
	...
	<form method="POST" action="<?php echo base_url('group/action_edit/'.md5($records->group_id)); ?>" class="m-form m-form--fit m-form--label-align-right form_add" >
			...
			<div class="row">
				<div class="col-md-6">
					<div class="form-group m-form__group row">
						...
						<div class="col-7">
							<input required class="form-control m-input" type="text" placeholder="Nama Group" id="example-text-input" name="nama_group" value="<?php echo $records->group_nama ?>">
							<input class="form-control m-input" type="hidden" placeholder="" name="param_role" value="<?php echo $records->role_id ?>">
					...
					<div class="form-group m-form__group row">
						...
						<div class="col-7">
							<textarea required class="form-control m-input" id="exampleTextarea" rows="3" placeholder="Deskripsi" name="deskripsi"><?php echo $records->group_deskripsi ?></textarea>
						</div>
					</div>
				</div>
				<div class="col-md-6">				
					<ul style="list-style: none;">
						<?php
							$group_data	= json_decode($records->group_data);
							...
							<label class="m-checkbox m-checkbox--solid m-checkbox--brand">
								...
								<?php foreach ($group_data as $key2 => $value2) {
									if ($value->menu_id == $value2->menu_id) {
							...
							<?php if (!empty($value->menu_sub_menu)): ?>
								<ul style="list-style-type: none;margin-left: 1.5%;" class="collapse <?php echo (!empty($value->menu_sub_menu) ? 'show' : ''); ?>" id="<?php echo $value->menu_id ?>">
								<?php $menu_sub_menu = json_decode($value->menu_sub_menu);
								foreach ($menu_sub_menu as $key2 => $value2): ?>
										<li style="margin-bottom: 1%;margin-top: 1%">
											<label class="m-checkbox m-checkbox--solid m-checkbox--brand">
												<input type="checkbox" value="<?php echo $value2->text ?>" class="<?php echo $value->menu_id ?>" onchange="cek_checked(this)" name="text[]" data-controller="<?php echo $value2->controller ?>" data-icon_menu ="<?php echo $value2->icon_menu ?>" data-parent="<?php echo $value2->parent ?>"
												<?php
													foreach ($group_data as $key4 => $value4) {
														$obj [] = $value4->menu_sub_menu;
													}

													foreach ($obj as $key5 => $value5) {
														if ($obj[$key5] != '') {
															foreach ($obj[$key5] as $key6 => $value6) {
																if ($value6->text == $value2->text) {
																	echo 'checked="checked"';
																}
															}
														}
